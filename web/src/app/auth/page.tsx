...
export default function AuthPage() {
  ...
  const [isLoading, setIsLoading] = useState<string | null>(null); // 'login', 'signup', 'google', 'github', 'guest'
  const [showPassword, setShowPassword] = useState({ login: false, signup: false });
  const [messages, setMessages] = useState<{ 
    login: { type: 'success' | 'error', text: string } | null,
    signup: { type: 'success' | 'error', text: string } | null 
  ...
  useEffect(() => {
    if (errorParam) {
      const errorMsg = errorParam === 'CredentialsSignin' 
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessages(prev => ({ ...prev, signup: { type: 'success', text: 'Account created! Logging in...' } }));
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-start p-4 md:p-8 relative overflow-hidden">
      ...
      >
        ...
        <h1 className="text-4xl font-extrabold text-white mb-3 tracking-tight">Get Started with MockMate</h1>
        <p className="text-slate-400 text-lg max-w-md mx-auto">One-click access or traditional account creation</p>
      </motion.div>

      {/* Social & Guest Section - NOW AT TOP */}
      <motion.div 
        initial={{ y: 20, opacity: 0 }}
        animate={{ y: 0, opacity: 1 }}
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <button
            ...
            className="flex items-center justify-center gap-3 py-4 border border-white/5 rounded-2xl hover:bg-white/5 hover:border-indigo-500/30 transition-all text-slate-300 font-semibold bg-slate-900/50 shadow-xl group"
          >
          ...
          <button
            ...
            disabled={isLoading !== null}
            className="flex items-center justify-center gap-3 py-4 border border-white/5 rounded-2xl hover:bg-white/5 hover:border-blue-500/30 transition-all text-slate-300 font-semibold bg-slate-900/50 shadow-xl group"
          >
        ...
        <button
          ...
          disabled={isLoading !== null}
          className="w-full flex items-center justify-center gap-3 py-4 text-slate-400 hover:text-indigo-400 border border-dashed border-white/10 hover:border-indigo-500/40 rounded-2xl transition-all group bg-slate-900/30"
        >
          {isLoading === 'guest' ? <Loader2 className="w-5 h-5 animate-spin" /> : (
            <>
        ...
        <div className="flex items-center gap-4">
          ...
          <span className="text-slate-600 text-sm font-bold uppercase tracking-widest px-4">OR USE EMAIL</span>
          <div className="flex-1 h-px bg-white/5" />
        </div>
      </motion.div>
            ...
            <AnimatePresence>
              {messages.login && (
                <motion.div 
                  ...
                  className={`p-4 rounded-xl text-sm flex gap-3 ${messages.login.type === 'error' ? 'bg-rose-500/10 text-rose-400 border border-rose-500/20' : 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20'}`}
                >
                  {messages.login.type === 'error' ? <XCircle size={18} /> : <CheckCircle2 size={18} />}
                  {messages.login.text}
                </motion.div>
              )}
            </AnimatePresence>
            ...
            <AnimatePresence>
              {messages.signup && (
                <motion.div 
                  initial={{ opacity: 0, scale: 0.95 }}
                  animate={{ opacity: 1, scale: 1 }}
                  className={`p-4 rounded-xl text-sm flex gap-3 ${messages.signup.type === 'error' ? 'bg-rose-500/10 text-rose-400 border border-rose-500/20' : 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20'}`}
                >
                  {messages.signup.type === 'error' ? <XCircle size={18} /> : <CheckCircle2 size={18} />}
      ...
      >
        ...
        <span className="text-slate-500 hover:text-white cursor-pointer px-1 transition-colors underline decoration-slate-800">Terms of Service</span> 
        and 
        <span className="text-slate-500 hover:text-white cursor-pointer px-1 transition-colors underline decoration-slate-800">Privacy Policy</span>
      </motion.p>
    </div>
  );
}
